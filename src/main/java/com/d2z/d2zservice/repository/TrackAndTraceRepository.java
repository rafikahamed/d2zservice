...
//	 List<String> getArticleId();
	...
	/* @Query(nativeQuery = true, value="SELECT DISTINCT t.articleid  from trackandtrace t where  t.user_id in('52','94') "
	 ...
	 @Modifying(flushAutomatically = true,clearAutomatically = true)
		@Transactional
		 @Query(nativeQuery = true, value="Update Trackandtrace set fileName = :status where  connoteNo in (:fastwayOrderId) and trackEventCode = 'SA'")
	void updateForPFLSubmitOrder(List<String> fastwayOrderId,String status);

	 @Modifying(flushAutomatically = true,clearAutomatically = true)
		@Transactional
		 @Query(nativeQuery = true, value="Update Trackandtrace set fileName = 'PFLSubmitOrderCompleted' where  fileName = 'PFLSubmitOrder'")
	
	void updateForPFLSubmitOrderCompleted();
