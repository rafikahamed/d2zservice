...
public class ETowerWrapper {
	...
	public void makeCreateShippingOrderEtowerCallForAPIData(CreateConsignmentRequest data,List<SenderDataResponse> senderDataResponseList) throws EtowerFailureResponseException {
	    ...
	    status = parseCreateShippingOrderResponse(response,senderDataResponseList,barcodeMap,gainLabelTrackingNo);
        }
		...
		if(gainLabelTrackingNo.size() == data.getConsignmentData().size()) {
		    ...
		    new Thread(r).start();
				...
				if(null!=status) {
				    ...
				    int userId = d2zDao.fetchUserIdbyUserName(data.getUserName());
					String senderFileID = d2zDao.createConsignments(data.getConsignmentData(),userId,data.getUserName(),barcodeMap);
	...
	public void makeCreateShippingOrderEtowerCallForFileData(List<SenderData> data,List<SenderDataResponse> senderDataResponseList) throws EtowerFailureResponseException {
		List<com.d2z.d2zservice.model.etower.CreateShippingRequest> eTowerRequest = constructEtowerRequestWithFileData(data);
		String status = null;
		Map<String, LabelData> barcodeMap= new HashMap<String,LabelData>();
		List<String> gainLabelTrackingNo = new ArrayList<String>();
	...
	private List<CreateShippingRequest> constructEtowerRequestWithFileData(List<SenderData> data) {
		...
		while(iterator.hasNext()) {
			...
			if("1PM3E".equalsIgnoreCase(orderDetail.getServiceType())) {
				...
				String zoneId = postCodeZoneMap.get(orderDetail.getConsigneeSuburb().trim().concat(orderDetail.getConsigneePostcode().trim()));
				if(weight>3) {
	...
	private String parseCreateShippingOrderResponse(CreateShippingResponse response,List<SenderDataResponse> senderDataResponseList,	
			Map<String,LabelData>  barcodeMap,List<String> gainLabelTrackingNo) throws EtowerFailureResponseException {
	...
	private Map<String, LabelData> processGainLabelsResponse(GainLabelsResponse response,Map<String,LabelData> barcodeMap) {
		List<ETowerResponse> responseEntity = new ArrayList<ETowerResponse>();
		 if(response!=null) {
